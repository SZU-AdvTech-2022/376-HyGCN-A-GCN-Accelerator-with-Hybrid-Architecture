...
void Controller::ClockTick() {
    ...
    for (int i = 0; i < config_.ranks; i++) {
        if (channel_state_.IsRankSelfRefreshing(i)) {
            ...
            if (all_idle) {
                simple_stats_.IncrementVec("all_bank_idle_cycles", i); // speedtest
                channel_state_.rank_idle_cycles[i] += 1;
            } else {
                simple_stats_.IncrementVec("rank_active_cycles", i); // speedtest
                // reset
    ...
    simple_stats_.Increment("num_cycles"); // speedtest
    return;
}
