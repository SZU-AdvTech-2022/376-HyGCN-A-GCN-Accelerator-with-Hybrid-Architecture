...
//static tp::ThreadPoolOptions op(4, false);
//std::shared_ptr<tp::ThreadPool> BaseDRAMSystem::thread_pool = std::make_shared<tp::ThreadPool>(op);
...
//        std::vector<std::future<void>> nmsl;
//        nmsl.reserve(ctrls_.size());
//
//        const int step_num = 4;
//        const int step_len = 4;
//        for(int i = 0; i < step_num; i++) {
//            std::packaged_task<void()> task([=]() {
//                for(int j = 0; j < step_len; j++) {
//                    int id = i * step_len + j;
//                    this->ctrls_[id]->ClockTick();
//                }
//            });
//            nmsl.push_back(task.get_future());
