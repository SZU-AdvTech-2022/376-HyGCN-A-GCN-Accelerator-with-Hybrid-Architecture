...
// calculated stats, similar to double, but not the same
    std::map<std::string, double> calculated_;
    std::map<std::string, uint64_t> counters_;
    std::map<std::string, double> doubles_;
   private:
    using VecStat = std::map<std::string, std::vector<uint64_t> >;
    using HistoCount = std::map<int, uint64_t>;
    ...
    std::map<std::string, std::string> header_descs_;

    // counter stats, indexed by their name

    std::map<std::string, uint64_t> epoch_counters_;
    ...
    std::map<std::string, std::vector<double> > vec_doubles_;



    // histogram stats
    std::map<std::string, std::vector<std::string> > histo_headers_;

    std::map<std::string, std::pair<int, int> > histo_bounds_;
    std::map<std::string, int> bin_widths_;
    std::map<std::string, HistoCount> histo_counts_;
    std::map<std::string, HistoCount> epoch_histo_counts_;
    VecStat histo_bins_;
